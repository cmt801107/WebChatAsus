...
var WebChat =
{
    isGetLocation: false,                                    //是否取得當前瀏覽器座標

    CRMGatewayUrl: "https://icr.asus.com/gateway/",   //自定義CRMGateway網址

    ECPUrl: "https://icr.asus.com/ecp/",       //自定義ECP網址
    ...
    doLoad: function () {
        ...
        //Survey Modal Init.
        $("#dvSurveyModalBody").html(Survey.getForm(WebChat.isEverInAgentService));
        ...
        $("#SurveyButton").click(function () { WebChat.showSurvey(); });
    ...
    memberlogin: function () {
        var jdo = $.Deferred();
        $.ajax({
            url: 'https://www.asus.com/api/asus_api_data.aspx?lang=tw&key=wechat0001',
            dataType: "jsonp",
            jsonp: 'callback',
            cache: false,
    ...
    doSendMessage: function (SendMessage) {
        ...
        if (WebChat.isInAgentService == false) {
            WebChat.sessionCounter.restart();
        }
    },
    ...
    startChat: function () {
        ...
        WebChat.ajax({
            ...
            data: {
                ...
                treeId: WebChat.treeId == null ? "001.001.001" : WebChat.treeId,
            ...
            success: function (ret) {
                ...
                if (ret.senderType.toLowerCase() == 'textivr') { //from ChatIVR 
                    ...
                    //提前建立websocket , 檔用戶輸入9999轉人工的時候，就不用再建立了
    ...
    sendMessage: function (message, elementId) {
        //Util.log("message: " + JSON.stringify(message) + " " + "elementId: " + elementId);
    ...
    getTenantInfo: function () {
        ...
        WebChat.ajax({
            ...
            data: {tenantId:WebChat.tenantId},
    ...
    processTextContent: function (text) {
        ...
        text = text.replace(/=[“]/g, "=\"").replace(/[”]]/g, "\"]")
               ...
               // .replace(/(http|ftp|https)_/gi, "<label 4 action=\"OpenUrl\" onclick=\"LinkAction.doOpenUrl('$1');\">$1</label>")
    ...
    doSocketAgentStop: function (data) {
        ...
        //if (data.reason != null && data.reason.toLowerCase() == "timeout") {
        //    WebChat.addSystemMessage(data.reason);
        //}
        //else {
        WebChat.addSystemMessage(data.reason);
       // }
...
window.unload = window.onbeforeunload = window.onunload= WebChat.doBeforeUnload;
