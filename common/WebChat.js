...
var WebChat =
{
    ...
    doLoad: function () {
        ...
        WebChat.needlogin = clientData.urlArgs.login;
    ...
    memberlogin: function () {
	    ...
	    if(WebChat.tenantId == "EN-SA" || WebChat.tenantId == "EN-AE"){
	    	lang = "eg";
		}else if(WebChat.tenantId == "NL-BE"){
			lang = "nl";
		}
        //console.log(lang);
        $.ajax({
    ...
    stopChat: function () {
        ...
        //console.log(WebChat.chatId);

        if (WebChat.chatId != null) {
           // console.log('callstopchat');
            WebChat.ajax({
                url: WebChat.CRMGatewayUrl + "openapi/web/chat/stop",
                async 	: false,
                data: { chatId: WebChat.chatId },
                                ...
                                if (Util.getCookie('defaultlogin') == 'true') {
                                    ...
                                    $.when(WebChat.memberlogin_withdefaultlogin()).done(function () {
                                        if (WebChat.loginSuccess == true) {
                                            ...
                                            if (WebChat.Onqueue) {
                                                // setTimeout(function(){WebChat.submitUserMessage(data, message, elementId)}, 1000);
                                                WebChat.addSystemMessage(WebChat.tenantInfo.onqueueHint);
                                            } else {
                                                WebChat.Onqueue = true;
                        ...
                        WebChat.ajax({
                            ...
                            success: function (ret) {
                                ...
                                if (ret.needAgent) {
                                    ...
                                    //預防Exception事件發生，若回傳的content包含Exception就自動重啟交談
                                    if (ret.content.indexOf("Exception") != -1) {
                                        $("#MessageList").html("");
                                        $('.modal-popup .pop-content ol li').removeClass('feedback-active');
                                        WebChat.startChat();
                                    }
    ...
    addMessage: function (message) {
        ...
        WebChat.activeTime = new Date().getTime();
    ...
    processTextContent: function (text) {
        ...
        text = text.replace(/=[“]/g, "=\"").replace(/[”]]/g, "\"]")
                ...
                .replace("http", " http").replace("ftp://", " ftp://").replace("\" http","\"http").replace("\" ftp","\"ftp")
                //action pattern
                ...
                //.replace(/(^|[^"'=])((http|https|ftp):\/\/([\w-]+\.)+[\w-]+([\w-.\/?=;!*%$]*)?([\w-&=;!*%$]*)?)/gi, "<label 3 action=\"OpenUrl\" onclick=\"window.open('$2');\">$2</label>")
                .replace(/(^|[^"'=])((http|https|ftp):\/\/([\w-]+\.)+[\w-]+(:?[\w-.\/?=)?([\w-&=;\@!*%$]*)?)/gi, "<label  action=\"OpenUrl\" onclick=\"window.open('$2');\">$2</label>")
    ...
    doSocketClose: function (reason) {
        //console.log('socket closed: ' + JSON.stringify(reason));
         	if(!WebChat.isNormalStop && WebChat.activeTime!=null){
        	var now = new Date().getTime();
            if((now-WebChat.activeTime)/1000/60>WebChat.tenantInfo.chatTimeoutMinutes){
				WebChat.addSystemMessage(WebChat.tenantInfo.gwChatTimeoutHint.replace("#{0}", WebChat.tenantInfo.chatTimeoutMinutes));
				WebChat.socket.options.reconnect = false;
			//	WebChat.socket.close();
		//		WebChat.socket = null;
				WebChat.isInAgentService = false;
				WebChat.chatId = null;
				WebChat.activeTime = null;
				WebChat.refreshButtonStatus();
			}
    ...
    doSocketMessageSend: function (data) {
        ...
        WebChat.ajax({
            url:  WebChat.CRMGatewayUrl + "openapi/web/message/isReceived",
            data: { messageId: data.messageId, isReceived: true },
            async: false,
            error: null,
            success: function (ret) {

                //console.log("message is confirmed : " + ret.confirmed);
...
window.onbeforeunload = WebChat.doBeforeUnload;
//window.onunload = WebChat.stopChat;
